...
.. code-block:: bash
    ...
    $ cp graphwhy/models/user.py graphwhy/models/blog_record.py
...
when we run ``initialize_graphwhy_db``.
...
``initialize_graphwhy_db`` command to update the database.

.. code-block:: bash

    $ $VENV/bin/initialize_graphwhy_db development.ini
